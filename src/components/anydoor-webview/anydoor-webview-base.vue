...
<script lang="ts" setup>
	...
	const emits = defineEmits(['pageStart', 'pageReady', 'loadResource', 'intercept',
		...
		"onConsole", 'pageError', 'httpError', 'sslError', "onNewWindow", "onLinkPress", "onImagePress",
		"onLinkClick", "onImageClick", "onBack", "onScheme"
	])
	...
	//新窗口
	const onNewWindow = (e) => {
		emits("onNewWindow", e.detail);
	}
	//链接长按
	const onLinkPress = (e) => {
		emits("onLinkPress", e.detail);
	}
	//有src的图片长按
	const onImagePress = (e) => {
		emits("onImagePress", e.detail);
	}
	//链接点击事件
	const onLinkClick = (e) => {
		emits("onLinkClick", e.detail);
	}
	//图片点击事件
	const onImageClick = (e) => {
		emits("onImageClick", e.detail);
	}
	//返回
	const onBack = (e) => {
		emits("onBack", e.detail);
	}
	//拦截scheme
	const onScheme = (e) => {
		emits("onScheme", e.detail);
	}
	//插件try catch捕获的错误
	const pluginError = (e) => {
		console.log(e)
	...
	const getUserAgent = () => {
		return new Promise((resolve, reject) => {
			if (mWebview.value === null) reject("初始化未完成!")
			mWebview.value.getUserAgent("", (response) => {
				resolve(response)
			})
		})
	}
	...
	const setUserAgent = (s: string) => {
		return new Promise((resolve, reject) => {
			...
			if (s === "") {
				reject("不能为空!")
			} else mWebview.value.setUserAgrnt(s, (response) => {
				resolve(response)
			})
		})
	}
	...
	const webConfig = computed(() => {
		...
		//是否启用多窗口
		const multipleWindow: string = props.config.multipleWindow === true ? "1" : "0"
		//window.open打开新窗口 用监听
		const windowsAutomatically: string = props.config.windowsAutomatically === true ? "1" : "0"
		//自动聚焦,默认关闭
		const autoFocus: string = props.config.windowsAutomatically === true ? "1" : "0"
		//拦截scheme,默认开启
		const scheme: string = props.config.scheme === false ? "0" : "1"
