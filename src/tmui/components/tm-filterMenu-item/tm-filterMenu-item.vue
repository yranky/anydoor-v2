<template>
    <scroll-view @click.stop="close"    scroll-y v-if="isActive&&FilterMenuMaxHeight>0&&!_props.isButton" :style="{height:FilterMenuMaxHeight+'px'}" class="overflow">
		<tm-divider :margin="[0,0]"></tm-divider>
		<view  @click.stop="stopEvent" ref="FilterMenuItemBody" class="FilterMenuItemBody" >
		   <tm-sheet :eventPenetrationEnabled="true" :color="props.bgColor" :margin="[0,0]" :padding="[32,32]">
			   <slot></slot>
		   </tm-sheet>
		</view>
	</scroll-view>
...
const activeIndex = inject("activeIndex",computed(()=>0));
const FilterMenuMaxHeight = inject("FilterMenuMaxHeight",computed(()=>0));
const AllList:ComputedRef<FilterMenuType[]> = inject("AllList",computed(()=>[]));
