<template>
	<list :style="{width: `${defaultProps.width}rpx`,height: `${defaultProps.height}rpx`}">
		<header v-if="_showHeader" class="tableBox flex flex-row"  :style="{width: `${defaultProps.width}rpx`,height: `${defaultProps.headerHeight}rpx`}">
			<view v-if="_fixed.length>0&&defaultProps.showFixed">
				<tm-sheet  :border="_showBottomBorder?1:0" border-direction="bottom" :color="_fixed[0].bgColor" :text="_fixed[0].light"
					...
					>
				    <view :userInteractionEnabled="false" :style="{width:_fixed[0].width-10+'rpx',height:defaultProps.headerHeight-6+'rpx'}" 
					...
					>	
						...
						<view  v-if="_fixed[0].sort" class="flex flex-col flex-col-center-center">
							<tm-icon :lineHeight='11'  :_class='(_fixed[0].sortType=="asce"||_fixed[0].sortType=="none"?"":"opacity-6")' :font-size="20" name='tmicon-sort-up'></tm-icon>
							<tm-icon :lineHeight='11' :_class='(_fixed[0].sortType=="desc"||_fixed[0].sortType=="none"?"":"opacity-6")' :font-size="20" name='tmicon-sort-down'></tm-icon>
						</view>
					</view>
				</tm-sheet>
			</view>
			...
			<scroller ref="scrolleHead" :scrollable="false" class="tableRight flex flex-row flex-1" scroll-direction="horizontal">
				<view class="flex flex-row">
					...
					<view
					v-for="(item, index) in _col" 
					:key="index">
						<tm-sheet  
						v-if="item.key!=_fixedKey"
						:border="_showBottomBorder?1:0" border-direction="bottom" :color="item.bgColor" :text="item.light"
							...
							>
						    <view  :userInteractionEnabled="false" :style="{width:item.width-10+'rpx',height:defaultProps.headerHeight-6+'rpx'}" 
							class="flex flex-row-center-center flex-row"
							:class="[item.align]"
							>	
								<view>
								...
								<view  v-if="item.sort" class="flex flex-col flex-col-center-center">
									<tm-icon :lineHeight='11'  :_class='(item.sortType=="asce"||item.sortType=="none"?"":"opacity-6")' :font-size="20" name='tmicon-sort-up'></tm-icon>
									<tm-icon :lineHeight='11'  :_class='(item.sortType=="desc"||item.sortType=="none"?"":"opacity-6")' :font-size="20" name='tmicon-sort-down'></tm-icon>
								</view>
							</view>
						</tm-sheet>
					</view>
		...
		<cell class="tableBox flex flex-row" :style="{width: `${defaultProps.width}rpx`}">
			<view v-if="_fixed.length>1&&defaultProps.showFixed">
				<view v-for="(item,index) in _fixed"
					:key="index">
					<tm-sheet  :border="_showBottomBorder?1:0" border-direction="bottom"
					v-if="index!=0"
					:color="item.bgColor" :text="item.light"
						:_class="'flex flex-col ' + item.align"
						...
						@click="headerClick(item.key,item.sort)"
						>
					    <view :userInteractionEnabled="false" :style="{width:item.width-10+'rpx',height:defaultProps.cellHeight-6+'rpx'}" 
						class="flex flex-row-center-center flex-row"
						:class="[item.align]"
						>	
							<view>
			...
			<scroller  ref="scrollerRight" class="tableRight flex-1 flex-row flex" scroll-direction="horizontal">
				<view >
					<view v-for="(item2, index2) in _tabel" :key="index2" class="flex-row flex" >
						<block  v-for="(item, index) in item2.data" :key="index">
							<tm-sheet :border="_showBottomBorder?1:0" 
							v-if="item.key!=_fixedKey"
