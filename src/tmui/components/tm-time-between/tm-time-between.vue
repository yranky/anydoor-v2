	...
	<tm-sheet :margin="[0,0]" :padding="[0,0]">
		<view v-if="props.quickBtn.length>0">
			...
			<view class="flex flex-row flex-row-top-start flex-wrap">
				<tm-tag  :color="props.color" @click="quickChangeClick(item)" v-for="(item,index) in props.quickBtn" :key="index" text outlined :shadow="0" :label="item.label"></tm-tag>
			</view>
		</view>
		...
		<view class="flex flex-row flex-row-center-center  px-16">
			<tm-sheet text @click="changeFocus(0)" :round="24" class="flex-1" parenClass="flex-1" _class="flex flex-row flex-row-center-center" 
			:color="focusIndex==0?props.color:'grey-4'" :height="76" outlined :border="4"  :margin="[0,0]" 
			:padding="[32,0]">
			...
			<tm-sheet text @click="changeFocus(1)"  :round="24" class="flex-1" parenClass="flex-1" _class="flex flex-row flex-row-center-center"  
			:color="focusIndex==1?props.color:'grey-4'" :height="76" outlined :border="4"  :margin="[0,0]" 
			:padding="[32,0]">
				<view :userInteractionEnabled="false" style="width:200rpx" class="flex flex-row flex-row-center-center">
		...
		<view>
			<tm-time-view :showDetail="props.showDetail" :format="props.format" :immediateChange="props.immediateChange" :start="starTtime" :end="endTtime" @change="timeChange" v-model="picker_focus_date" :defaultValue="picker_focus_date"></tm-time-view>
		</view>
...
<script lang="ts" setup>
	...
	function timeChange(evt:string){
		...
		// #ifdef APP-NVUE
		checkTimeVaild()
		let nst = DayJs(valuedate_start.value).isValid()?DayJs(valuedate_start.value).format(props.format):""
		let ese = DayJs(valuedate_end.value).isValid()?DayJs(valuedate_end.value).format(props.format):""
		if(props.asyncModel){
	...
	function setDate(ar:Array<String|Number|Date>){
		if(Array.isArray(ar)){
			if(ar.length==2){
				...
				valuedate_start.value = DayJs(ar[0]).isValid()?DayJs(ar[0]).format(defaultFormat):''
				valuedate_end.value  = DayJs(ar[1]).isValid()?DayJs(ar[1]).format(defaultFormat):''
	...
	function confirm(){
		let nst = valuedate_start.value?DayJs(valuedate_start.value).format(props.format):""
