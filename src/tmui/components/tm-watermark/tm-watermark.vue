<template>
  <view>
    ...
    <view class="overflow fixed" style=" left: -999px">
      <!-- #ifdef APP-NVUE -->
      <gcanvas id="waterCanvas" ref="waterCanvasRef"
               :style="{ width: `${boxInfo.width}${props.unit}`, height: `${boxInfo.height}${props.unit}` }"></gcanvas>
      <!-- #endif -->
      <!-- #ifdef MP-WEIXIN || MP-ALIPAY || MP-QQ -->
      <canvas id="waterCanvas" type="2d"
              :style="{ width: `${boxInfo.width}${props.unit}`, height: `${boxInfo.height}${props.unit}` }"></canvas>
      <!-- #endif -->
      <!-- #ifndef MP-WEIXIN || MP-ALIPAY || MP-QQ || APP-NVUE -->
      <canvas id="waterCanvas" canvas-id="waterCanvas"
              :style="{ width: `${boxInfo.width}${props.unit}`, height: `${boxInfo.height}${props.unit}` }"></canvas>
      <!-- #endif -->
    </view>
...
async function init() {
  ...
  // #endif
  // #ifndef MP-WEIXIN || MP-ALIPAY || MP-QQ || APP-NVUE
  const ctx = uni.createCanvasContext("waterCanvas", proxy),canvas=null;
  // #endif
  ...
  if (ctx) {
    ...
    if (image) {
          ...
          imageObj.onload = () => {
            ...
            ctx.drawImage(imageObj, canvasOffsetLeft, canvasOffsetTop,(props.imageWidth ||
                (imageHeight
                    ? (imageObj.width * imageHeight) / imageObj.height
                    : imageObj.width)) * ratio, (props.imageHeight ||
                (imageWidth
                    ? (imageObj.height * imageWidth) / imageObj.width
                    : imageObj.height)) * ratio);
            endInit({ctx: ctx, canvas: canvas ?? null, canvasWidthPx: canvasWidthPx, canvasHeightPx: canvasHeightPx});
          }
          // #endif
      ...
      await uni.getImageInfo({
        ...
        success: function (img) {
          ctx.globalAlpha = props.imageOpacity;
          ctx.drawImage(image, canvasOffsetLeft, canvasOffsetTop, (props.imageWidth ||
              (imageHeight
              ...
              (imageWidth
                  ...
                  : img.height)) * ratio);
          endInit({ctx: ctx, canvas: canvas, canvasWidthPx: canvasWidthPx, canvasHeightPx: canvasHeightPx});
        }
      });
          // #endif
...
function endInit({
                   ...
                   canvasWidthPx,
                   canvasHeightPx
                 }: { ctx: any, canvas: any, canvasWidthPx: any, canvasHeightPx: any }){
  // #ifdef APP-NVUE
  ctx.draw(false,()=> {
