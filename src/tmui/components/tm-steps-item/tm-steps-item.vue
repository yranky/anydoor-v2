<template>
	<view :class="[
		'relative flex-1',
		'flex flex-col flex-col-center-center overflow',
		...
		<view  class="flex-1 flex-row flex-row-center-between absolute" :style="{ width: _width,top:(nodeSize/2)+'rpx'}">
			...
			<view :style="{ width: _widthLine}">
				<tm-divider :border="2" v-if="_cureent<_countCurrent" :color="_color"  :followDark="props.followDark" :dark="props.dark"  :margin="[0, 0]">
				</tm-divider>
			</view>
		</view>
		<!-- 内容 区域。 -->
		<view @click.stop="stepClick" class="flex flex-col relative" style="justify-content: flex-start;align-items: center;">
			<tm-sheet :eventPenetrationEnabled="true" no-level  :followTheme="props.followTheme" :followDark="props.followDark"
				...
				<tm-text v-if="!_icon && !status && _typeModel != 'dot'" :userInteractionEnabled="false"
					:font-size="22" :label="_cureent + 1"></tm-text>
				<tm-icon :font-size="22"  v-if="status && _isActive && _typeModel != 'dot'"
					:name="status.icon"></tm-icon>
				<tm-icon :font-size="22"  v-if="!status && _icon && _typeModel != 'dot'" :name="_icon">
				</tm-icon>
			</tm-sheet>
			<view :eventPenetrationEnabled="true"  style="width:120rpx" class="flex flex-col flex-col-center-center  mt-12">
				<slot name="default">
					<tm-text v-if="props.title"  :userInteractionEnabled="false"
						:color="_isCheck ? _activeColor : ''" _class="text-overflow-2" :font-size="24" :label="props.title">
					</tm-text>
					<tm-text v-if="props.label"  :userInteractionEnabled="false"
