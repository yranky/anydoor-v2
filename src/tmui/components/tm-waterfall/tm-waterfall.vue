<template>
	<view>
		<view class="flex flex-col flex-col-top-start  flex-between relative overflow" :style="[{width:_containerWidth+'rpx',height:(_containerHeight.maxHeight+50)+'px'}]">
			<slot></slot>
		</view>
		<!-- 虚拟加载占位符。 -->
		<tm-sheet v-if="_totalNum.length!=_list.length"  _class="flex flex-center" :margin="[0,0]" :padding="[0,0]">
			<tm-icon name="tmicon-loading" spin></tm-icon>
		</tm-sheet>
	</view>
</template>
