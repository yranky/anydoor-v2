	...
	>
		<refresh  @refresh="onrefresh" @pullingdown="onpullingdown" :display="showLoading ? 'show' : 'hide'">
		    <slot name="pull" :status="{refreshStatus}">
		    	<view class="flex flex-row flex-row-center-center" :style="{height:(_barHeight/2)+'px',opacity:refreshStatus ==2||refreshStatus ==3?1:0}">
		    	...
		    	<view v-if="refreshStatus != -1 && refreshStatus != 2"
		    		...
		    		:style="{opacity:`${refreshStatus==0?(_barHeight/nowEvt.viewHeight):1}`,height:(_barHeight/2)+'px'}">
